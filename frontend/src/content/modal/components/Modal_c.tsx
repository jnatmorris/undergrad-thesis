...
export const Modal_c: React.FC<Props> = ({ startSending }) => {
    ...
    const { serverState, isSocketConnected } = useSocketStore();
    ...
    const opacityChange = classnames(
        ...
        { "opacity-100": !serverState.isServerCalc && isSocketConnected },
        {
            "cursor-not-allowed":
                serverState.isServerCalc || !isSocketConnected,
        },
        { "opacity-50": serverState.isServerCalc || !isSocketConnected },
        { "cursor-pointer": !serverState.isServerCalc && isSocketConnected }
    );
    ...
    return (
        <div
            id="input"
            className="group invisible absolute left-0 top-0 bg-white/[97] opacity-0 transition-all duration-[450ms] target:visible target:opacity-95"
        >
            ...
            <div className="rounded-lg pt-1 shadow-md ring-1 ring-slate-200 backdrop-blur-lg">
